...
public class Reporte {
    public void generarReporte(String rutaPDF) throws BadElementException, IOException {
        ...
        try (Connection conexion = conexionBD.crearConexion()) {
            String consulta = "SELECT cm.id, cm.FechaCompra, c.Nombre AS NombreCliente, "
                    + "p.Titulo, s.Nombre AS Sucursal, f.dia, f.HoraInicio, f.HoraFin "
                    + "FROM compra cm "
                    + "INNER JOIN funciones f ON cm.id_funcion = f.id "
            ...
            try (PreparedStatement ps = conexion.prepareStatement(consulta); ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    agregarCelda(tabla, "Compra", String.valueOf(rs.getInt("Id_Compra")));
